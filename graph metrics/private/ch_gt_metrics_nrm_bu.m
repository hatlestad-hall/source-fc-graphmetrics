...
function [ gt_emp, gt_emp_nrm, gt_rand ] = ch_gt_metrics_nrm_bu ( adjc_mat, null_itr )
...
gt_emp.node.clustering_coeff			= clustering_coef_bu ( adjc_mat );
...
gt_emp.network.transitivity = transitivity_bu ( adjc_mat );

% Find the number of connected components.
ch_verbose ( 'Connected components...', 0, 1, 10 );
gt_emp.node.components		= get_components ( adjc_mat );
...
gt_emp.network.assortativity = assortativity_bin ( adjc_mat, 0 );

% Compute the rich-club coefficient.
ch_verbose ( 'Rich-club coefficient...', 0, 1, 10 );
gt_emp.node.rich_club = rich_club_bu ( adjc_mat );
...
gt_emp.node.local_efficiency = efficiency_bin ( adjc_mat, 1 );

% Compute betweenness centrality.
ch_verbose ( 'Betweenness centrality...', 0, 1, 10 );
[ gt_emp.node.edge_betweenness_centrality, gt_emp.node.betweenness_centrality ] = edge_betweenness_bin ( adjc_mat );
...
for r = 1 : null_itr
	...
	gt_rand.node( r ).clustering_coeff = clustering_coef_bu ( rand_mat );
	gt_rand.network( r ).clustering_coeff = mean ( gt_rand.node( r ).clustering_coeff );
	gt_rand.network( r ).transitivity = transitivity_bu ( rand_mat );
	gt_rand.node( r ).components = get_components ( rand_mat );
	...
	gt_rand.network( r ).assortativity = assortativity_bin ( rand_mat, 0 );
	gt_rand.node( r ).rich_club = rich_club_bu ( rand_mat );
	rand_mat_dst = distance_bin ( rand_mat );
	...
	gt_rand.node( r ).local_efficiency = efficiency_bin ( rand_mat, 1 );
	[ gt_rand.node( r ).edge_betweenness_centrality, gt_rand.node( r ).betweenness_centrality ] = edge_betweenness_bin ( rand_mat );
